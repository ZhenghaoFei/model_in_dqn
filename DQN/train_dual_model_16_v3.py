# This version utilize all possible performance improve method
# resnet skip connection
# Q(lambda)
# batchnorm
# match nonliearlity with paper
# decrease size 
...
SUMMARY_DIR = './results_dual_m_16_v3/'
RANDOM_SEED = 1234
...
class QNetwork(object):
    ...
    def create_Q_network(self):
        ...
        state_f = layers.convolution2d(state, num_outputs=16, kernel_size=3, stride=1, padding='SAME', activation_fn=None)
        state_f = layers.convolution2d(state_f, num_outputs=16, kernel_size=3, stride=1,padding='SAME', activation_fn=None)
        state_f = layers.batch_norm(state_f)
        state_f = tf.nn.relu(state_f)

        # model 1 from state feature to action number of next state
        state_m1_h1 = layers.convolution2d(state_f, num_outputs=16, kernel_size=3, stride=1, padding='SAME', activation_fn=None)
        state_m1_h1 = layers.convolution2d(state_m1_h1, num_outputs=16, kernel_size=3, stride=1, padding='SAME', activation_fn=None)
        state_f = layers.batch_norm(state_f)
        state_f = tf.nn.relu(state_f)

        state_m1_n = layers.convolution2d(state_m1_h1, num_outputs=32, kernel_size=3, stride=1, padding='SAME', activation_fn=None)
        state_m1_n = layers.convolution2d(state_m1_n, num_outputs=self.a_dim, kernel_size=3, stride=1, padding='SAME', activation_fn=None)

        reward_m1_n = layers.fully_connected(layers.flatten(state_m1_h1), num_outputs=32, activation_fn=None)
        reward_m1_n = layers.fully_connected(reward_m1_n, num_outputs=self.a_dim, activation_fn=None)
        ...
        value_w0 = tf.Variable(np.random.randn(dim, ch_h), dtype=tf.float32)
        value_b0 = tf.Variable(tf.zeros([ch_h]), dtype=tf.float32, name="value_b")
        value_w1 = tf.Variable(np.random.randn(ch_h, ch_latent_actions), dtype=tf.float32)
        value_b1 = tf.Variable(tf.zeros([ch_latent_actions]), dtype=tf.float32, name="value_b")

        # gamma(discount rate)  function
        gamma_w0 = tf.Variable(np.random.randn(dim, ch_h), dtype=tf.float32)
        gamma_b0 = tf.Variable(tf.zeros([ch_h]), dtype=tf.float32, name="gamma_b")
        gamma_w1 = tf.Variable(np.random.randn(ch_h, ch_latent_actions), dtype=tf.float32)
        gamma_b1 = tf.Variable(tf.zeros([ch_latent_actions]), dtype=tf.float32, name="gamma_b")

        # lambda(discount rate)  function
        lambda_w0 = tf.Variable(np.random.randn(dim, ch_h), dtype=tf.float32)
        lambda_b0 = tf.Variable(tf.zeros([ch_h]), dtype=tf.float32, name="lambda_b")
        lambda_w1 = tf.Variable(np.random.randn(ch_h, 1), dtype=tf.float32)
        ...
        for i in range(self.a_dim):
            ...
            for j in range(k):
                # state
                state_m2_h1 = tf.nn.conv2d(state_n, m2_w0, strides=(1, 1, 1, 1), padding='SAME') + m2_b0
                state_m2_ns = tf.nn.conv2d(state_m2_h1, m2_w1, strides=(1, 1, 1, 1), padding='SAME') + m2_b1
                state_m2_ns = layers.batch_norm(state_m2_h1)
                ...
                # gamma
                gamma_n = tf.matmul(flat_state_m2_h1, gamma_w0) +gamma_b0
                gamma_n = tf.nn.sigmoid(tf.matmul(gamma_n, gamma_w1) +gamma_b1)

                # value
                value_n = tf.matmul(flat_state_m2_h1, value_w0) + value_b0
                value_n = tf.matmul(value_n, value_w1) + value_b1

                # labmda
                lambda_n = tf.matmul(flat_state_m2_h1, lambda_w0) +lambda_b0
                ...
                state_nt = tf.expand_dims(state_nt, 3)
                state_n += state_nt
                state_n /= 2
                ...
                q_a += (1-lambda_n) * discount_reward_n + lambda_n * discount_value_n
