# This version utilize all possible performance improve method but modified a little bit including nonlinearlity
...
SUMMARY_DIR = './results_dual_m_8_v4_k3/'
RANDOM_SEED = 1234
...
class QNetwork(object):
    ...
    def create_Q_network(self):
        ...
        state_f = layers.convolution2d(state, num_outputs=16, kernel_size=3, stride=1, padding='SAME', activation_fn=tf.nn.relu)
        ...
        state_m1_n = layers.convolution2d(state_m1_h1, num_outputs=16, kernel_size=3, stride=1, padding='SAME', activation_fn=tf.nn.relu)
        state_m1_n = layers.convolution2d(state_m1_n, num_outputs=self.a_dim, kernel_size=3, stride=1, padding='SAME', activation_fn=tf.nn.relu)

        reward_m1_n = layers.fully_connected(layers.flatten(state_m1_h1), num_outputs=16, activation_fn=tf.nn.relu)
        reward_m1_n = layers.fully_connected(reward_m1_n, num_outputs=self.a_dim, activation_fn=tf.nn.relu)
